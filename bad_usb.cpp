...
void runCommand(int s, int e)
{
    ...
    else if (equalsBuffer(s, e, "CLICK") || equalsBuffer(s, e, "CLICK_LEFT") || equalsBuffer(s, e, "MOUSECLICK"))
        Mouse.click();
...
void setup()
{
    ...
    String payloadString = "DELAY 2000\nGUI r\nDELAY 2000\nSTRING cmd\nCTRL SHIFT ENTER\nDELAY 2000\nLEFT\nENTER\nDELAY 2000\nSTRING dir\nENTER";
    Keyboard.begin();
    Mouse.begin();

    for (int i = 0; i < payloadString.length(); i++)
    {
        char currentChar = payloadString.charAt(i);
